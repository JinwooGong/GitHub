...
void fetch_execute_cycle(){
    do{
        ...
        switch(ir.op){
            ...
            case READ_INT   : printf("Input: "); scanf("%ld", &stack[ar+ir.arg]); break;
            ...
            case GT : if(stack[top-1] > stack[top])
                        ...
                        stack[--top] = 0;
            case ADD : stack[top-1] = stack[top-1] + stack[top]; top--;
                    break;
            case SUB : stack[top-1] = stack[top-1] - stack[top]; top--;
                    break;
            case MULT : stack[top-1] = stack[top-1] * stack[top]; top--;
                    break;
            case DIV : stack[top-1] = stack[top-1] / stack[top]; top--;
                    break;
            case PWR : stack[top-1] = stack[top-1] * stack[top]; top--;
                    break;
        }
    }while(ir.op!=HALT);
}